...
public class BuyRentController {
	...
	public String buyRent(HttpServletRequest request, Model model) {
		User user = (User) request.getSession().getAttribute("user");
		if (user == null)
			return "login";
		List<RentingOrder> rentingOrders = rentingOrderService.getRentingOrdersByUserId(user.getId());
		List<SellingOrder> sellingOrders = sellingOrderService.getSellingOrdersByUserId(user.getId());
